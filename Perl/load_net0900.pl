...
load_net0900 - This script will load the excel report net0900 into tables vo.net0900_in_cmdb_work and vo.net0900_not_in_cmdb_work.

=head1 VERSION HISTORY

version 1.0 23 July 2013 DV
...
This script will load the report net0900 into the table vo.net0900_in_cmdb_work and vo.net0900_not_in_cmdb_work.

=head1 SYNOPSIS

 load_net0900.pl [-x net0900.xls file]

 load_net0900 -h	Usage
 load_net0900 -h 1  Usage and description of the options
 load_net0900 -h 2  All documentation

=head1 OPTIONS

=over 4

=item B<-x Full path to NET0900 file in .xls format>

Full path and file to the NET0900 report. The report needs to be in .xls format, since the Perl application cannot handle .xlsx files.
...
# Truncate tables
my @tables = qw (net0900_in_cmdb_work net0900_not_in_cmdb_work);
...
import_sheet($worksheet, $dbh, "net0900_in_cmdb_work", 6, "NET0900");

# Then get spider data not linked to CMDB
my $worksheet = @worksheets[1];
$worksheet_name = $worksheet->get_name;
$log->info("Ready to load worksheet: $worksheet_name");
import_sheet($worksheet, $dbh, "net0900_not_in_cmdb_work", 7, "NET0900");
